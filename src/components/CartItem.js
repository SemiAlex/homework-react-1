...
function CartItem({product, addCount, reduceCount, removeFromCart}) {
    return <ListGroup.Item key={product.id}>
        ...
        <Badge bg="secondary" className='pointer' onClick={() => reduceCount(product.id)}>{product.count}</Badge>
        <Badge bg="danger" className='pointer' onClick={() => reduceCount(product.id)}>-</Badge>
        <Badge bg="danger" className='pointer mgl' onClick={() => removeFromCart(product.id)}>remove</Badge>
    </ListGroup.Item>
}
